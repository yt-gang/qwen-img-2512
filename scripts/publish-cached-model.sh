#!/usr/bin/env bash
# Publish the exact Qwen Image 2512 worker model set as a private HF repository.
set -euo pipefail

SOURCE_MODEL_ROOT="${SOURCE_MODEL_ROOT:-/workspace/models}"
STAGING_ROOT="${STAGING_ROOT:-/workspace/qwen-image-2512-runpod-cache}"
...
See models.json in https://github.com/yt-gang/qwen-img-2512 for pinned revisions and checksums.
EOF
...
# Qwen Image 2512 RunPod cache

Private, minimal cached-model package for `yt-gang/qwen-img-2512`. The worker's
`models.json` is the authoritative list of pinned files, sizes and SHA-256 values.
EOF
