#!/usr/bin/env bash
# Model validation and downloading script for Qwen-Image-2512
...
declare -A MODELS=(
    ["diffusion_models/qwen_image_2512_fp8_e4m3fn.safetensors"]="Comfy-Org/Qwen-Image_ComfyUI|split_files/diffusion_models/qwen_image_2512_fp8_e4m3fn.safetensors|diffusion_models|Qwen Image 2512 Diffusion Model (fp8)"
...
main() {
    ...
    if [[ ${#missing_models[@]} -gt 0 ]]; then
        ...
        for model_path in "${missing_models[@]}"; do
            local model_info="${MODELS[$model_path]}"
            local repo_id file_path relative_path model_name
